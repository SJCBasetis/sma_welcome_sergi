...
def fancy_string_time_from_seconds(input_seconds):
    ...
    output = ""
    for time, abr in [(hours, "h"),(minutes, "m"),(seconds, "s")]:
        if time > 0 :
            output += "{}{}".format(time, abr)
...
def create_file_headers(uri_log):
    ...
    if not os.path.isfile(uri_log):
        ...
        columns = ["Module", "Type", "Date", "Details", "Time",
                   "Error Type", "Error Details", "Error line"]
        df = DataFrame(columns=columns)
        df.set_index("Module").to_csv(uri_log, sep=";")
